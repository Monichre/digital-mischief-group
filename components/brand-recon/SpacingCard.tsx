...
export function SpacingCard({ spacing }: SpacingCardProps) {
  ...
  const baseUnitValue = spacing.unit ? parseInt(spacing.unit.replace(/[^\d]/g, '')) : null
  ...
  return (
    <div className="border border-zinc-800 bg-zinc-900/30 p-6 relative">
      ...
      {spacing.unit && (
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-mono text-zinc-300">{spacing.unit}</span>
          </div>
        </div>
      )}
      ...
      {baseUnitValue && (
        <div className="mt-6">
          <div className="text-[10px] uppercase tracking-wider text-zinc-600 mb-3">Unit Multipliers</div>
          <div className="flex items-end gap-2">
            {[1, 2, 3, 4, 6, 8].map((multiplier) => (
              <div key={multiplier} className="flex flex-col items-center gap-1">
                ...
                />
                <span className="text-[9px] text-zinc-600">{baseUnitValue * multiplier}</span>
              </div>
            ))}
          </div>
        </div>
