...
import { Globe, Loader2, Link2, ImageIcon, FileText, Code, Download, Copy, CheckCircle2 } from "lucide-react"
        ...
        <Card className="bg-zinc-900/50 border-zinc-800">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-zinc-100">{scrapedData.title || "Untitled"}</CardTitle>
                <p className="text-zinc-500 text-sm font-mono mt-1">{scrapedData.url}</p>
              </div>
              ...
              <TabsContent value="content" className="mt-4">
                {scrapedData.description && (
                  <div className="mb-4 p-3 bg-zinc-950/50 rounded-lg">
                    <span className="text-zinc-500 font-mono text-xs">DESCRIPTION</span>
                    <p className="text-zinc-300 text-sm mt-1">{scrapedData.description}</p>
                  </div>
                )}
                {scrapedData.content && (
                  <div className="p-3 bg-zinc-950/50 rounded-lg max-h-96 overflow-y-auto">
                    <span className="text-zinc-500 font-mono text-xs">EXTRACTED TEXT</span>
                    <p className="text-zinc-300 text-sm mt-2 whitespace-pre-wrap">{scrapedData.content}</p>
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="links" className="mt-4">
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {scrapedData.links?.map((link, i) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-zinc-950/50 rounded">
                      <Link2 className="w-3 h-3 text-zinc-500 flex-shrink-0" />
                      <div className="min-w-0 flex-1">
                        <p className="text-zinc-300 text-sm truncate">{link.text || "(no text)"}</p>
                        <p className="text-zinc-600 text-xs font-mono truncate">{link.href}</p>
                      </div>
                    </div>
                  ))}
                  {(!scrapedData.links || scrapedData.links.length === 0) && (
                    <p className="text-zinc-500 text-center py-4 font-mono text-sm">NO LINKS FOUND</p>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="images" className="mt-4">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
                  {scrapedData.images?.map((img, i) => (
                    <div key={i} className="bg-zinc-950/50 rounded overflow-hidden">
                      ...
                      <p className="text-zinc-500 text-xs p-2 truncate font-mono">{img.alt || img.src}</p>
                    </div>
                  ))}
                  {(!scrapedData.images || scrapedData.images.length === 0) && (
                    <p className="text-zinc-500 text-center py-4 font-mono text-sm col-span-full">NO IMAGES FOUND</p>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="metadata" className="mt-4">
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {scrapedData.metadata &&
                    Object.entries(scrapedData.metadata).map(([key, value], i) => (
                      <div key={i} className="flex items-start gap-2 p-2 bg-zinc-950/50 rounded">
                        <Badge
                          variant="outline"
                  ...
                  {(!scrapedData.metadata || Object.keys(scrapedData.metadata).length === 0) && (
                    <p className="text-zinc-500 text-center py-4 font-mono text-sm">NO METADATA FOUND</p>
                  )}
                </div>
              </TabsContent>
            </Tabs>
