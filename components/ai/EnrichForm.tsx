...
import { Loader2, Sparkles, Building2, Users, Globe, TrendingUp, Linkedin, Twitter } from "lucide-react"
...
export const companyEnrichmentSchema = z.object({
  ...
  socialPresence: z
    ...
    .optional(),
  enrichmentScore: z.number().min(0).max(100).describe("Data completeness score"),
})
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex gap-2">
              ...
              >
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "ENRICH"}
              </Button>
            </div>
            <div className="flex gap-2">
              <Badge
                variant="outline"
                className={`cursor-pointer ${inputType === "url" ? "bg-orange-500/20 border-orange-500" : "border-zinc-700"}`}
              >
                URL
              </Badge>
              <Badge
                variant="outline"
                className={`cursor-pointer ${inputType === "email" ? "bg-orange-500/20 border-orange-500" : "border-zinc-700"}`}
              >
                Email
              </Badge>
              <Badge
                variant="outline"
                className={`cursor-pointer ${inputType === "text" ? "bg-orange-500/20 border-orange-500" : "border-zinc-700"}`}
              >
                Company
              </Badge>
            </div>
        ...
        <div className="grid gap-4">
          ...
          {object.company && (
            <Card className="bg-zinc-900/50 border-zinc-800">
              ...
              <CardContent className="space-y-4">
                {object.company.description && <p className="text-zinc-400 text-sm">{object.company.description}</p>}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  {object.company.industry && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">INDUSTRY</span>
                      <p className="text-zinc-200">{object.company.industry}</p>
                    </div>
                  )}
                  {object.company.headquarters && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">HQ</span>
                      <p className="text-zinc-200">{object.company.headquarters}</p>
                    </div>
                  )}
                  {object.company.employeeRange && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">SIZE</span>
                      <p className="text-zinc-200">{object.company.employeeRange}</p>
                    </div>
                  )}
                  {object.company.fundingStage && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">FUNDING</span>
                      <p className="text-zinc-200">{object.company.fundingStage}</p>
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="grid gap-2">
                  {object.leadership.map((person, i) => (
                    <div key={i} className="flex items-center justify-between p-2 bg-zinc-950/50 rounded">
                      <div>
                        <p className="text-zinc-200 text-sm">{person.name}</p>
                        <p className="text-zinc-500 text-xs">{person.title}</p>
                      </div>
                      {person.linkedin && <Linkedin className="w-4 h-4 text-zinc-500" />}
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {object.technologies && object.technologies.length > 0 && (
              <Card className="bg-zinc-900/50 border-zinc-800">
                <CardHeader>
                  <CardTitle className="text-zinc-100 text-sm font-mono">TECH STACK</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            {object.competitors && object.competitors.length > 0 && (
              <Card className="bg-zinc-900/50 border-zinc-800">
                <CardHeader>
                  <CardTitle className="text-zinc-100 text-sm font-mono">COMPETITORS</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-wrap gap-2">
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 gap-4">
                  {object.marketPosition.strengths && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">STRENGTHS</span>
                  ...
                  {object.marketPosition.opportunities && (
                    <div>
                      <span className="text-zinc-500 font-mono text-xs">OPPORTUNITIES</span>
                      <ul className="mt-2 space-y-1">
