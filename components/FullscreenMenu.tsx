...
const MENU_ITEMS = [
  { label: "Enrich", href: "/enrich", number: "01", icon: Sparkles, description: "Multi-agent company intelligence" },
  { label: "Research", href: "/research/live", number: "02", icon: Brain, description: "Real-time intelligence synthesis" },
  { label: "Brand Recon", href: "/brand-recon", number: "03", icon: Palette, description: "Extract brand identity from any URL" },
...
export function FullscreenMenu({ isOpen, onClose }: FullscreenMenuProps) {
  ...
  const parallaxX = (mousePosition.x / (typeof window !== "undefined" ? window.innerWidth : 1) - 0.5) * 20
  const parallaxY = (mousePosition.y / (typeof window !== "undefined" ? window.innerHeight : 1) - 0.5) * 20
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("absolute inset-0 transition-opacity duration-1000", isOpen ? "opacity-100" : "opacity-0")}
        style={{
      ...
      >
        ...
        {[...Array(8)].map((_, i) => (
          <div
            key={`v-${i}`}
            className={cn("absolute top-0 w-px transition-all duration-1000 ease-out", isOpen ? "h-full" : "h-0")}
            style={{
              left: `${(i + 1) * 12.5}%`,
              transitionDelay: `${i * 50}ms`,
              background: `linear-gradient(to bottom, transparent, rgba(234, 88, 12, ${0.05 + (i % 3) * 0.05}), transparent)`,
            }}
          />
        ))}
        {/* Horizontal lines */}
        {[...Array(6)].map((_, i) => (
          <div
            key={`h-${i}`}
            className={cn("absolute left-0 h-px transition-all duration-1000 ease-out", isOpen ? "w-full" : "w-0")}
            style={{
              top: `${(i + 1) * 16.66}%`,
              transitionDelay: `${i * 50 + 100}ms`,
              background: `linear-gradient(to right, transparent, rgba(234, 88, 12, ${0.05 + (i % 3) * 0.05}), transparent)`,
            }}
          />
        ))}
      </div>
      ...
      {[
        ...
        { pos: "bottom-8 left-8", border: "border-l-2 border-b-2", delay: "500ms" },
        { pos: "bottom-8 right-8", border: "border-r-2 border-b-2", delay: "550ms" },
      ].map((corner, i) => (
        <div
          key={i}
          className={cn(
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs tracking-[0.3em] text-zinc-400 font-mono">DIGITAL MISCHIEF GROUP</span>
        </div>
      </Link>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  transform: isOpen ? `translateY(0) translateX(${isHovered ? 20 : 0}px)` : "translateY(48px)",
                }}
              >
                {/* Number */}
                <span
                ...
                <div
                  className={cn(
                    ...
                    isHovered ? "border-orange-500 bg-orange-500/10 scale-110" : "border-zinc-700 bg-zinc-900/50",
                  )}
                >
                  <Icon
                    className={cn("w-5 h-5 transition-all duration-300", isHovered ? "text-orange-500" : "text-zinc-500")}
                  />
                  {isHovered && <div className="absolute inset-0 border border-orange-500 animate-ping opacity-50" />}
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <span
                    className={cn(
                      ...
                      isHovered ? "text-orange-500/70 translate-x-0 opacity-100" : "text-zinc-600 -translate-x-2 opacity-0",
                    )}
                  >
                    {item.description}
                  </span>
                ...
                <ArrowRight
                  className={cn(
                    ...
                    isHovered ? "text-orange-500 opacity-100 translate-x-0" : "text-orange-500/30 opacity-0 -translate-x-8",
                  )}
                />
