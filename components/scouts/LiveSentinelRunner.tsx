...
import { Play, Loader2, CheckCircle, AlertCircle, Brain, TrendingUp, Target } from "lucide-react"
import { Button } from "@/components/ui/button"
import type { SentinelStreamEvent, AIInsight, CompetitiveIntel, TrendSignal } from "@/lib/scouts/stream-types"
...
export function LiveSentinelRunner({ scoutId, scoutName, onComplete }: LiveSentinelRunnerProps) {
  ...
  const hasResults = insights.length > 0 || competitiveIntel.length > 0 || trends.length > 0

  return (
    <div className="space-y-6">
      {/* Control Panel - Open Scouts Style */}
      <div className="border border-zinc-200 bg-white p-6 rounded-lg">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-zinc-900">{scoutName}</h3>
          ...
          <Button
            ...
            disabled={isRunning}
            className={isRunning ? "bg-zinc-400" : "bg-[#fa5d19] hover:bg-[#e54d0f] text-white"}
          >
            {isRunning ? (
              <>
        ...
        {/* Search Progress */}
        {isRunning && <SearchProgressIndicator searchProgress={searchProgress} />}
      ...
      {/* AI Thinking Stream */}
      {isRunning && events.some((e) => e.type === "ai_thought") && (
        <AIThinkingStream events={events.filter((e) => e.type === "ai_thought")} />
      )}
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="border border-zinc-200 bg-white p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-zinc-700">Insights</span>
            </div>
            <p className="text-2xl font-bold text-zinc-900">{insights.length}</p>
          </div>

          <div className="border border-zinc-200 bg-white p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <Target className="w-4 h-4 text-[#fa5d19]" />
              <span className="text-sm font-medium text-zinc-700">Competitive Intel</span>
            </div>
            <p className="text-2xl font-bold text-zinc-900">{competitiveIntel.length}</p>
          </div>
      ...
      {insights.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-zinc-900">AI-Generated Insights</h4>
      ...
      {competitiveIntel.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-zinc-900">Competitive Intelligence</h4>
          {competitiveIntel.map((intel, i) => (
            <div key={i} className="border border-zinc-200 bg-white p-4 rounded-lg">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-medium text-zinc-900">{intel.competitor_name || "Unknown Competitor"}</p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-zinc-900">{trend.trend_name}</p>
                    {trend.emerging && (
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded">Emerging</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-xs text-zinc-600">{trend.strength}%</span>
                  </div>
                </div>
              </div>
            </div>
