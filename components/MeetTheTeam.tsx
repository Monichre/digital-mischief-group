...
import {SniperScope} from './effects/SniperScope'
...
export function MeetTheTeam() {
  ...
  const handleNavigate = (direction: 'prev' | 'next') => {
    ...
    const currentIndex = TEAM_MEMBERS.findIndex(
      ...
      <div className='relative max-w-4xl mx-auto'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8'>
          {TEAM_MEMBERS.map((member, index) => (
            ...
            >
              {/* Card Container */}
              <div className='relative aspect-square overflow-hidden border border-white/10 bg-void/50 backdrop-blur-sm transition-all duration-500 group-hover:border-red-500/50 group-hover:shadow-[0_0_30px_rgba(239,68,68,0.2)]'>
                {/* Sniper Scope Targeting Effect */}
                <SniperScope isActive={hoveredId === member.id} />

                {/* Corner Accents - hidden when sniper scope is active */}
                <div
                  className={`absolute top-0 left-0 w-4 h-4 border-t border-l border-copper/30 transition-all duration-300 ${
                    hoveredId === member.id
                      ? 'opacity-0'
                      : 'group-hover:w-6 group-hover:h-6 group-hover:border-copper'
                  }`}
                />
                <div
                  className={`absolute top-0 right-0 w-4 h-4 border-t border-r border-copper/30 transition-all duration-300 ${
                    hoveredId === member.id
                      ? 'opacity-0'
                      : 'group-hover:w-6 group-hover:h-6 group-hover:border-copper'
                  }`}
                />
                <div
                  className={`absolute bottom-0 left-0 w-4 h-4 border-b border-l border-copper/30 transition-all duration-300 ${
                    hoveredId === member.id
                      ? 'opacity-0'
                      : 'group-hover:w-6 group-hover:h-6 group-hover:border-copper'
                  }`}
                />
                <div
                  className={`absolute bottom-0 right-0 w-4 h-4 border-b border-r border-copper/30 transition-all duration-300 ${
                    hoveredId === member.id
                      ? 'opacity-0'
                      : 'group-hover:w-6 group-hover:h-6 group-hover:border-copper'
                  }`}
                ...
                {/* Click Indicator - changes based on hover state */}
                <div className='absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-30'>
                  <div
                    className={`flex items-center gap-2 text-xs tracking-wider ${
                      hoveredId === member.id ? 'text-red-500' : 'text-copper'
                    }`}
                  >
                    <span
      ...
      {isModalOpen && selectedMember && (
        ...
        >
          ...
          <div
            ...
            className={`relative w-full h-full max-w-5xl max-h-[90vh] mx-4 flex flex-col md:flex-row items-stretch transition-all duration-500 ${
          ...
          >
            ...
            <div className='w-full md:w-80 p-8 border-t md:border-t-0 md:border-l border-copper/20 flex flex-col justify-center bg-void/50 backdrop-blur-sm'>
              <div className='space-y-6'>
                ...
                {/* Name */}
                <div>
                  <div className='text-gray-500 text-xs tracking-wider mb-1'>
                    IDENTITY
                  </div>
                  <h3 className='text-white text-3xl font-mono tracking-wide'>
                    {selectedMember.name}
                  </h3>
                </div>
                ...
                <div>
                  ...
                  <p className='text-copper text-sm tracking-wider uppercase'>
                    {selectedMember.role}
                  </p>
                </div>
                ...
                <div className='pt-4 border-t border-white/10'>
                  <p className='text-gray-600 text-xs tracking-wide'>
                    Move mouse over particles to interact. Click for explosion
                    effect.
                  </p>
                </div>
