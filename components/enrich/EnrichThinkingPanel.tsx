...
import type { PhaseState, ConductorThought, ConductorDecision } from '@/hooks/useEnrichStream'
...
export function EnrichThinkingPanel({ phases, events, thoughts = [], decisions = [], isComplete, synthesis }: EnrichThinkingPanelProps) {
  const scrollRef = useRef<HTMLDivElement>(null)
  const [viewMode, setViewMode] = useState<'conductor' | 'pipeline'>('conductor')
  ...
  const renderThought = (thought: ConductorThought, index: number) => {
    ...
    return (
      <div key={index} className="flex items-start gap-2 p-2.5 rounded bg-zinc-800/50 border border-zinc-700/50">
        ...
        <div className="flex-1 min-w-0">
          ...
          {thought.relatedPhase && (
            <span className="text-[10px] text-zinc-600 mt-1 inline-block">
              → {PHASE_CONFIG[thought.relatedPhase as keyof typeof PHASE_CONFIG]?.label || thought.relatedPhase}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center gap-1 p-0.5 bg-zinc-800 rounded text-xs">
          <button
            ...
            className={`px-2 py-1 rounded transition-colors ${
              viewMode === 'conductor' ? 'bg-orange-500/20 text-orange-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            Thinking
          </button>
          <button
            ...
            className={`px-2 py-1 rounded transition-colors ${
              viewMode === 'pipeline' ? 'bg-orange-500/20 text-orange-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            Pipeline
          </button>
            ...
            return (
              <div 
                ...
                className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs flex-shrink-0 ${
                  state.status === 'running' ? 'bg-orange-500/20 text-orange-400' :
                  state.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                  state.status === 'failed' ? 'bg-red-500/20 text-red-400' :
                  state.status === 'skipped' ? 'bg-zinc-800 text-zinc-500' :
                  'bg-zinc-800 text-zinc-500'
                }`}
              >
                {getStatusIcon(state.status)}
                <span className="truncate">{config?.label?.replace(' Agent', '') || phase}</span>
              </div>
            )
          })}
        </div>
        ...
        {viewMode === 'conductor' ? (
          ...
          <>
            {thoughts.length === 0 && !isComplete ? (
              ...
              <>
                {thoughts.map((thought, index) => renderThought(thought, index))}
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
            <div 
              ...
              style={{ 
                width: `${Object.values(phases).reduce((sum, p) => sum + (p.status === 'skipped' ? 100 : p.progress), 0) / Object.keys(phases).length}%` 
              }}
            />
          </div>
          <span className="text-xs text-zinc-500 font-mono">
            {Math.round(Object.values(phases).reduce((sum, p) => sum + (p.status === 'skipped' ? 100 : p.progress), 0) / Object.keys(phases).length)}%
          </span>
        </div>
      </div>
    </div>
