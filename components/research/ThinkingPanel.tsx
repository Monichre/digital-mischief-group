...
import { Brain, Search, Globe, FileText, Loader2, CheckCircle2, AlertCircle } from 'lucide-react'
...
export function ThinkingPanel({ events, isComplete }: ThinkingPanelProps) {
  ...
  const renderEvent = (event: ResearchStreamEvent, index: number) => {
    switch (event.type) {
        ...
        return (
          <div key={index} className="flex items-center gap-3 p-2 text-sm">
            ...
            <span className="text-zinc-400">
              Searching <span className="text-blue-400 font-mono">{event.data.source}</span>...
            </span>
          </div>
        )
        ...
        return (
          <div key={index} className="flex items-center gap-3 p-2 text-sm">
            ...
            <span className="text-zinc-300">
              <span className="text-blue-400 font-mono">{event.data.source}</span>
              {' '}returned {event.data.resultCount} results
              <span className="text-zinc-500 ml-2">({event.data.duration}ms)</span>
            </span>
          </div>
        )
      ...
      case 'synthesis_start':
        return (
          <div key={index} className="flex items-center gap-3 p-3 bg-orange-500/10 rounded border border-orange-500/30 mt-2">
      ...
      case 'complete':
        return (
          <div key={index} className="flex items-center gap-3 p-3 bg-green-500/10 rounded border border-green-500/30 mt-2">
      ...
      case 'error':
        return (
          <div key={index} className="flex items-center gap-3 p-3 bg-red-500/10 rounded border border-red-500/30 mt-2">
