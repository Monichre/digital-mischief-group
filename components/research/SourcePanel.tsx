...
import { ExternalLink, FileText, ChevronDown, ChevronUp, Globe, Loader2 } from 'lucide-react'
...
function SourceCard({ source }: { source: SourceFoundEvent['data'] & { isLoading?: boolean; fullContent?: string } }) {
  ...
  return (
    <div className={`rounded-lg border ${getSourceColor(source.source)} overflow-hidden transition-all`}>
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${getSourceBadgeColor(source.source)}`}>
                {source.source}
              </span>
              {source.isLoading && (
