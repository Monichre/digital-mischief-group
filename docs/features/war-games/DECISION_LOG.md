...
2. Change to `/field-reports` (as originally planned)
3. Use `/war-games` for actual page, `/field-reports` for navigation

**Decision**: Use `/war-games` for the actual page route, update all navigation to `/field-reports`

**Rationale**:

- `/war-games` is more descriptive of the feature
- `/field-reports` maintains brand consistency in navigation
