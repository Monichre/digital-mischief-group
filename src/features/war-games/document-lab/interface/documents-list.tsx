...
import { supabaseAdminClient } from "../lib/client"
...
export async function DocumentsList() {
  ...
  return (
    ...
    <div className="space-y-4 pr-4">
      <h2 className="pl-1 text-xs font-semibold text-muted-foreground">Recent Documents</h2>
      {documents.map((document) => (
        <DocumentCard
          key={document.id}
          document={document}
          tasks={tasks?.filter((task) => task.document_id === document.id) ?? []}
          chunks={chunks?.filter((chunk) => chunk.document_id === document.id) ?? []}
          entities={entities?.filter((entity) => entity.document_id === document.id) ?? []}
        />
      ))}
    </div>
    // </ScrollArea>
