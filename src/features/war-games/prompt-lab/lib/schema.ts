...
export const defaultExamples = {
  ...
  classification: {
    systemPrompt:
      "You are an expert customer support ticket classifier. Analyze the customer message and classify it into one of these categories: BILLING, TECHNICAL, ACCOUNT, PRODUCT, or GENERAL. Also provide the priority level (LOW, MEDIUM, HIGH, URGENT) based on the urgency and impact. Format your response as a JSON object with the exact structure shown in the examples.",
    examples: [
      {
        input:
  ...
  summarization: {
    systemPrompt:
      "You are an expert content summarizer. Create a concise 2-3 sentence summary of the given text, focusing on the key points and maintaining the original tone. Include one key action item or takeaway when relevant. Return your response as plain text (not JSON).",
    examples: [
      {
        input:
