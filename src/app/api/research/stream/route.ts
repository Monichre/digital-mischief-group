  ...
  const stream = new ReadableStream( {
    async start( controller ) {
      ...
      try {
        ...
        // Phase 2: Search multiple sources in parallel
        ...
        // Add Firecrawl results (already have scraped content!)
        ...
        // Phase 5: Synthesize with Claude (streaming)
        send( {
          type: 'thinking',
...
Synthesize these findings into an intelligence brief.`,
        ...
        try {
          const [mission] = await sql`
            INSERT INTO research_missions (
              ...
              ${`Live Research: ${query.slice( 0, 50 )}${query.length > 50 ? '...' : ''}`},
              ${query},
              'deep',
          ...
          console.error( '[Research Stream] DB save error:', dbError )
        }
        ...
        try {
          ...
          console.error( '[Research Stream] Usage log error:', usageError )
        }
