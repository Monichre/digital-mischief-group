...
export default function AppDownloadStack({
  ...
  const stackVariants = useMemo(
    () => ({
      initial: (i: number) => ({
        rotate: i % 2 === 0 ? -ROTATION_MULTIPLIER * (i + 1) : ROTATION_MULTIPLIER * (i + 1),
        x: i % 2 === 0 ? -TRANSLATION_MULTIPLIER * (i + 1) : TRANSLATION_MULTIPLIER * (i + 1),
        y: 0,
        ...
        <AnimatePresence mode="wait">
          ...
          {isExpanded && !isDownloading && (
            ...
            >
              <button
                ...
                type="button"
              >
                <p className="my-0 font-medium leading-0 text-white">{title}</p>
                <div className="flex items-center gap-1">
                  <p className="my-0 font-medium leading-0 text-white">{selected.length}</p>
                  ...
                  >
                    <div
                      className={`pointer-events-none absolute top-2 right-2 flex h-4 w-4 items-center justify-center rounded-full border border-solid ${
                        selected.includes(app.id) ? "border-orange-500 bg-orange-500" : "border-white/60"
                      }`}
                    >
                    ...
                    <button
                      className={`group flex h-full w-full flex-col items-center justify-center gap-1 rounded-xl border-2 border-transparent bg-zinc-900/80 p-2 transition-all duration-200 hover:border-orange-500 ${
                        selected.includes(app.id) ? "border-orange-500 bg-orange-500/10" : ""
                      }`}
                    ...
                    >
                      <img
                        ...
                        width={40}
                      />
                      <span className="font-medium text-white text-xs">{app.name}</span>
                    </button>
                  </motion.li>
                ))}
              </motion.ul>
              <button
                className="mt-4 w-full rounded-lg bg-orange-500 px-4 py-2 font-semibold text-white shadow transition hover:bg-orange-600 disabled:opacity-50"
          ...
          {isDownloading && (
            ...
            >
              ...
              <span className="font-semibold text-orange-500">Downloading...</span>
            </motion.div>
          )}
