  ...
  const processDocuments = async () => {
    ...
    for (const doc of pendingDocs) {
      ...
      try {
        ...
        setDocuments((prev) =>
          prev.map((d) =>
            d.id === doc.id ? {...d, status: 'completed', extractedData} : d
          )
        )
      } catch (error) {
        setDocuments((prev) =>
          prev.map((d) =>
            d.id === doc.id
              ? {...d, status: 'failed', error: (error as Error).message}
              : d
          )
        )
      }
          ...
          >
            ...
            <Upload
              className={`w-8 h-8 mx-auto mb-4 ${
                isDragActive ? 'text-orange-500' : 'text-zinc-500'
              }`}
            />
            {isDragActive ? (
          ...
          <CardContent className='space-y-4'>
            {documents
              ...
              .map((doc) => (
                <div
                  key={doc.id}
                  className='p-4 bg-zinc-950/50 rounded-lg space-y-3'
                >
                  <div className='flex items-center gap-2'>
                    {getFileIcon(doc.type)}
                  ...
                  {doc.extractedData?.entities &&
                    doc.extractedData.entities.length > 0 && (
                      <div className='flex flex-wrap gap-2'>
